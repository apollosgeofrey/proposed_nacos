...
//here the final verification and insertion is executed
        if ($mat_no != null && !empty($mat_no) && $new_email_address != null && !empty($new_email_address) && $phone_num != null && !empty($phone_num) && $gender != null && !empty($gender) && $form_good_state == 1) {
                ...
                if ($status == true) {
                    ...
                    $lisenced_by_mat = $_SESSION['email'];
                    ...
                    if ($update_pass_now_run == true) { 
                  	  	header("Location: nacos_dash.php?adder_mem=successfully&mat_client=$mat_no&mail_client=$new_email_address");
                    } else {
		...
		<div class="modal-dialog	">
			<div class="modal-content">
				...
				<div class="modal-header">
					...
					<h2 class="modal-title">Add a New Nacosite</h2>
				</div>
...
<!-- end of Modal -->
	...
	<div class="container">
		<div class="row">
			<div class="col-lg-12">
				<h2 class="pageTitle">NACOS Dashboard</h2>
			</div>
		</div>
	</div>
	</section>
...
<section id="content">
	<div class="container">
		...
		<h4 class="alert alert-success"><strong><center>
			<?php echo "<a href='upcoming.php?dfjsjks=sdsdsdk'>" . $db_surname . " " . $db_othernames . "</a>";  ?> , Welcome to NACOS Dashboard!
		</center></strong></h4><br>
		
		<?php 
			if (isset($_SESSION['rank'])) {
				if ($_SESSION['rank'] == '') {
					echo "<button data-toggle='modal' data-target='#popUpWindow' class='btn btn-success btn-md col-xs-6'>
						Add New Nacosite </button>";
				}
			}
		?>

		<a href="services.php?jhsdjksdjhkdhas=khsdjhasdjkdjksa"><button class="btn btn-success btn-md col-xs-6">NACOS Events</button></a>

		<a href="#duespayment.php?jusdfkjhsdjfsd=fsddjhsdjkhjs"><button class="btn btn-success btn-md col-xs-6">Departmental Dues</button></a>

		<a href="about.php#list_excos"><button class="btn btn-success btn-md col-xs-6">NACOS EXCOS</button></a>
